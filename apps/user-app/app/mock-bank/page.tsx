...
export default function MockBankPage() {
    ...
    async function onAuthorize() {
        ...
        const minLoadingTime = delay(5000)
        ...
        try {
            ...
            setTimeout(() => router.push(`/dashboard?onramp=1&token=${encodeURIComponent(token)}`), 1500);
        } catch (e: any) {
            await minLoadingTime;
            setResult("error");
    ...
    return (
        <>
            {submitting && (
                <Loader
                    message={result === "ok" ? "Payment Authorized" : "Securely communicating with bank..."}
                />
            )}
